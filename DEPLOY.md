# 🚀 ИНСТРУКЦИЯ ПО РАЗВЕРТЫВАНИЮ НА СЕРВЕРЕ

## 📋 Подготовка

### На вашем ПК (уже сделано):
✅ Изменены имена контейнеров: `v2_glpi`, `v2_chatwoot`, и т.д.
✅ Изменены домены: `glpi2.yapomogu.com`, `chat2.yapomogu.com`

### На сервере (нужно сделать):

## 🔧 Шаг 1: Создать бэкап старой системы

```bash
# Подключитесь к серверу
ssh user@your-server

# Перейдите в директорию старого проекта
cd /путь/к/helpdesk-stack-npm

# Создайте бэкап (НЕ ОСТАНАВЛИВАЯ систему!)
./backup.sh

# Проверьте что бэкап создан
ls -lh backups/
```

## 📦 Шаг 2: Загрузить новые файлы на сервер

**Вариант A: Git (если используете)**
```bash
# На сервере
cd /path/to/helpdesk-stack-npm
git pull origin main
```

**Вариант B: SCP/SFTP (если нет git)**
```bash
# На вашем ПК (из директории проекта)
# Проверьте что файлы готовы
ls -la

# Загрузите файлы на сервер
scp docker-compose.yml .env* *.sh README.md user@your-server:/path/to/helpdesk-stack-npm/

# Или весь проект
rsync -avz --exclude='data/' --exclude='backups/' ./ user@your-server:/path/to/helpdesk-stack-npm/
```

## 🌐 Шаг 3: Настроить DNS и NPM

### Настройка DNS:
Добавьте A-записи для новых доменов:
```
glpi2.yapomogu.com  ->  IP_вашего_сервера
chat2.yapomogu.com  ->  IP_вашего_сервера
```

### Настройка Nginx Proxy Manager:

**Для GLPI2:**
```
Domain Names:     glpi2.yapomogu.com
Scheme:          http
Forward Hostname: v2_glpi
Forward Port:     80
Block Common Exploits: ✅
Websocket Support: ❌
```

**SSL:**
- Request a new SSL Certificate (Let's Encrypt)
- Force SSL: ✅

**Для Chat2:**
```
Domain Names:     chat2.yapomogu.com
Scheme:          http
Forward Hostname: v2_chatwoot
Forward Port:     3000
Block Common Exploits: ✅
Websocket Support: ✅ ОБЯЗАТЕЛЬНО!!!
```

**SSL:**
- Request a new SSL Certificate (Let's Encrypt)
- Force SSL: ✅

## 🚀 Шаг 4: Запустить новую систему

```bash
# На сервере
cd /path/to/helpdesk-stack-npm

# Проверьте .env
cat .env
# Проверьте что домены changed to glpi2 и chat2

# Убедитесь что скрипты исполняемые
chmod +x *.sh

# Проверьте что сеть npm_default существует
docker network ls | grep npm_default

# Запустите базы данных
docker compose up -d v2_glpi_db v2_chatwoot_db v2_chatwoot_redis

# Подождите 60 секунд
sleep 60

# Проверьте что БД healthy
docker compose ps

# Выполните миграции Chatwoot
docker compose run --rm chatwoot bundle exec rails db:schema:load DISABLE_DATABASE_ENVIRONMENT_CHECK=1
docker compose run --rm chatwoot bundle exec rails db:seed

# Запустите все сервисы
docker compose up -d

# Проверьте статус
docker compose ps
```

## ✅ Шаг 5: Проверка

### Проверка контейнеров:
```bash
docker compose ps

# Должны быть запущены:
# v2_glpi            - healthy
# v2_glpi_db         - healthy
# v2_chatwoot        - healthy (или starting)
# v2_chatwoot_db     - healthy
# v2_chatwoot_redis  - healthy
# v2_chatwoot_sidekiq - healthy
```

### Проверка логов:
```bash
# GLPI
docker logs v2_glpi --tail 50

# Chatwoot
docker logs v2_chatwoot --tail 50

# Если есть ошибки
docker logs v2_chatwoot 2>&1 | grep -i error
```

### Проверка в браузере:
1. **GLPI**: https://glpi2.yapomogu.com
   - Должен открыться мастер установки
   - Настройки БД:
     - Сервер: `v2_glpi_db`
     - База: `glpi`
     - Пользователь: `glpi`
     - Пароль: (из `.env`)

2. **Chatwoot**: https://chat2.yapomogu.com
   - Должна открыться страница создания Super Admin
   - Создайте первый аккаунт администратора

## 🔄 Шаг 6: После установки GLPI

```bash
# Удалите installer для безопасности
docker exec v2_glpi rm /var/www/html/install/install.php
```

## 📊 Мониторинг

```bash
# Статус системы
./status.sh

# Использование ресурсов
docker stats v2_glpi v2_chatwoot v2_chatwoot_sidekiq

# Логи в реальном времени
docker compose logs -f
```

## 🆚 Сравнение старой и новой версии

| Параметр | Старая версия | Новая версия |
|----------|--------------|--------------|
| Контейнеры | `glpi`, `chatwoot` | `v2_glpi`, `v2_chatwoot` |
| Домены | glpi.yapomogu.com | glpi2.yapomogu.com |
| | chat.yapomogu.com | chat2.yapomogu.com |
| Хранение | `./data/` (bind mounts) | Named volumes |
| Безопасность | Пароли в файле | Пароли в .env |
| Health checks | ❌ Нет | ✅ Есть |
| Resource limits | ❌ Нет | ✅ Есть |

## 🔄 Миграция данных (опционально)

Если нужно перенести данные со старой версии:

```bash
# 1. Создайте бэкап старой системы
cd /path/to/old-version
./backup.sh

# 2. Скопируйте бэкап
cp backups/backup_YYYY-MM-DD_HH-MM.tar.gz /path/to/new-version/backups/

# 3. Восстановите в новую систему
cd /path/to/new-version
./restore.sh backup_YYYY-MM-DD_HH-MM.tar.gz
```

## 🛑 Остановка старой версии (после тестирования)

**ТОЛЬКО после полного тестирования новой версии!**

```bash
# Перейдите в старую директорию
cd /path/to/old-version

# Создайте финальный бэкап
./backup.sh

# Остановите старые контейнеры
docker compose down

# Освободите порты на NPM
# Удалите или отключите proxy для glpi.yapomogu.com и chat.yapomogu.com
```

## ⚠️ Откат (если что-то пошло не так)

```bash
# Остановите новую версию
docker compose down

# Запустите старую версию
cd /path/to/old-version
docker compose up -d
```

## 📝 Checklist развертывания

- [ ] Создан бэкап старой системы
- [ ] Файлы загружены на сервер
- [ ] DNS настроен для новых доменов
- [ ] NPM настроен для новых доменов с SSL
- [ ] WebSocket включен для chat2.yapomogu.com
- [ ] Базы данных запущены и healthy
- [ ] Миграции Chatwoot выполнены
- [ ] Все сервисы запущены
- [ ] GLPI установлен через веб-интерфейс
- [ ] Chatwoot Super Admin создан
- [ ] Installer GLPI удален
- [ ] Логи проверены на ошибки
- [ ] Система работает стабильно 24+ часа
- [ ] Старая версия остановлена (опционально)

## 🆘 Помощь

**Проблема: Контейнеры не запускаются**
```bash
# Проверьте логи
docker compose logs

# Проверьте ресурсы
docker system df
df -h

# Проверьте сеть
docker network ls
```

**Проблема: Chatwoot unhealthy**
```bash
# Скорее всего это нормально при первом запуске
# Подождите 2-3 минуты
# Проверьте логи
docker logs v2_chatwoot --tail 100
```

**Проблема: GLPI не открывается**
```bash
# Проверьте что контейнер запущен
docker ps | grep v2_glpi

# Проверьте NPM настройки
# Forward должен быть: v2_glpi:80
```

## 📞 Контакты

При возникновении проблем:
1. Проверьте логи: `docker compose logs`
2. Проверьте статус: `./status.sh`
3. Проверьте документацию: `cat README.md`
